  ...
  getters: {
    ...
    logOldValue(state) {
      if (state.log.old_value !== "null" && state.log.old_value !== undefined) {
        return Object.entries(JSON.parse(state.log.old_value))
          .map(([key, value]) => {
            return { key, value };
          })
          .filter(
      ...
      return new Promise((resolve, reject) => {
        ...
        log
          ...
          .then(response => {
            ...
            commit("SET_TOTAL_LOGS", response.count);
